...
## 3. 时间参数与角度计算
儒略日计算
儒略日的计算公式为 JD = 367Y - INT(7 * (Y + INT((M + 9) / 12)) / 4) + INT(275M / 9) + D + 1721013.5，其中 Y 为年份，M 为月份，D 为日期。

儒略世纪间隔 T = (JD - 2451545.0) / 36525.0。
儒略日间隔 d = JD - 2451545.0。
...
> **注意：**
> 在测试与使用中 `datetime_to_julian_params` 函数的输入参数 `imaging_time` 默认为 TT时
> 我不确定 DX_ZY3_NAD_att.txt 中的时间参数是否是TT时 我也不确定这个影响有多大
